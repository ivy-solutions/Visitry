...
Meteor.methods({
  ...
  removeUserFromAgency(userArgs){
    Errors.checkUserLoggedIn(this.userId, 'removeUserFromAgency', 'Must be logged in to remove a user from an agency.')
    Errors.checkUserIsAdministrator(this.userId, userArgs.agencyId, 'removeUserFromAgency', 'Must be an agency administrator to remove users from an agency.')
    ...
    user.userData.agencyIds = user.userData.agencyIds.filter((item) => {
      return item !== userArgs.agencyIds
    })
    user.save((err, id) => {
      if (err) {
